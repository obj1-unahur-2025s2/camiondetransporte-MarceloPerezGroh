...
object camion {
    ...
    method carga(unaCosa) = carga.add(unaCosa)
    
    method descarga(unaCosa) = carga.remove(unaCosa)

    method pesoTotal() = 1000 + carga.sum()
    
    //even para saber si es par

    method esCargaPar() = self.pesoTotal().even()
    
    method algunaCosaPesoEsto(unPeso) = carga.any({cosas => cosas.peso()== unPeso})
    ...
    method exedeElPeso() = carga.pesoTotal() >= 2500
